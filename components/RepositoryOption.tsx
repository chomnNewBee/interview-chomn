...
export function RepositoryOption(props:props) {
  return (
    <Combobox.Option
      //here need set value to url,otherwise example.tsx can't access url
        ...
        <>
          <header className="flex items-center">
            <CpuChipIcon
              ...
              aria-hidden="true"
            />
            <span className="ml-1 font-bold flex-auto truncate">{props.repo.name}</span>
          </header>
          ...
          <footer className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span className="flex items-center space-x-1">
                ...
                <span className="font-medium">{props.repo.owner.login}</span>
              </span>
              {/* Language */}
              <span className="flex items-center space-x-1">
                <span className="block w-1.5 h-1.5 rounded-full bg-indigo-400" />
                <span className="font-medium">{props.repo.language}</span>
              </span>
            </div>
            <div className="flex items-center space-x-2.5">
              ...
              <span className="flex items-center space-x-0.5">
                ...
                <span>{props.repo.open_issues_count}</span>
              </span>
              {/* Forks */}
              <span className="flex items-center space-x-0.5">
                <svg
